...
if($_SERVER ['REQUEST_METHOD'] == 'POST') {
  ...
  if (empty($error)) {
    ...
    if(mysqli_num_rows($result) == 0) {
      $q = "UPDATE pesakit SET FirstName='$n', LastName='$l', InsuranceNumber='$in', Diagnose='$d' WHERE ID_P='$id' LIMIT 1";
      $result = @mysqli_query($connect, $q);
...
$q = "SELECT FirstName, LastName, InsuranceNumber, Diagnose FROM pesakit WHERE ID_P=$id";
$result = @mysqli_query($connect, $q);
